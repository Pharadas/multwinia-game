...
## Flip a construction-site building to built: replaces the ghost mesh with
## the full-size opaque one.
func set_built(building: Node3D) -> void:
	...
	building.queue_free()
	var full := _make_building_mesh(bid, team, true)
	full.position = pos
